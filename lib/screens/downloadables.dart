...
class Downloadables extends StatefulWidget {
  static String routeName = 'downloadables';

  @override
  _DownloadablesState createState() => _DownloadablesState();
}
        ...
        if (snapshot.hasData) {
          ...
          return ListView.builder(
              ...
              itemBuilder: (context, index) {
                return VideoCardPay(VideoInfo.fromMap(data[index], index));
              });
        } else
...
class VideoCardPay extends StatefulWidget {
  ...
  @override
  _VideoCardPayState createState() => _VideoCardPayState();
}

class _VideoCardPayState extends State<VideoCardPay> {
  Color iconColor;
  bool isFav;
  ...
  Widget build(BuildContext context) {
    User user = Provider.of<UserAuthProvider>(context).authUser ??
        FirebaseAuth.instance.currentUser;
    return widget.videoInfo.pay.contains(user.uid)
        ? Container(
        ...
        : length == 0
            ? EmptyWidget(
                msg: 'There are no movies available for download',
              )
            : SizedBox.shrink();
  }
}
